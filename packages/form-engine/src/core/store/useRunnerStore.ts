...
export const useRunnerStore = create<RunnerState>((set, get) => ({
  ...
  updateField: (id, value) => set((state) => {
  ...
  validate: (fields) => {
    ...
    const checkField = (field: Field, value: any, path: string) => {
      ...
      if (value && field.validationRegex && !newErrors[path]) { try { if (!new RegExp(field.validationRegex).test(value)) { newErrors[path] = field.errorMsg || '格式不正确'; isValid = false; } } catch(e) {} }
