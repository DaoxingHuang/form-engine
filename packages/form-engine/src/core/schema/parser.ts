...
export const parseSchemaToFields = (schema: Schema): Field[] => {
  ...
  return Object.keys(properties).map(key => {
    ...
    if (prop.enum) {
       options = prop.enum.map((val: string, i: number) => ({ value: val, label: (prop.enumNames && prop.enumNames[i]) ? prop.enumNames[i] : val }));
    } else if (prop.items && prop.items.enum) {
       options = prop.items.enum.map((val: string, i: number) => ({ value: val, label: (prop.items.enumNames && prop.items.enumNames[i]) ? prop.items.enumNames[i] : val }));
    }
